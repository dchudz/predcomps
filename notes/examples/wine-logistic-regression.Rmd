...
- *Variation 3*: Just like *Variation 1*, but there is more variation in quality at the middle price ranges than the edges. This leads to quality having a larger APC, because the APC weights large changes more than small ones, and here large changes are happening in the region where the inverse logistic curve is steepest.
...
nRowsForApcSample <- 400

# This is used in determining the weights that go into computing the APC.
# The default value of 1 gives unintuitive results 
# (the difference between Variation 1 and 3 is not as expected using 1)
# We need to think more about the weights! Maybe default should not be 1.
...
```{r}
apc1 <- GetPredCompsDF(model=logitFit1, df=df1, numRowsToUse = nRowsForApcSample, mahalanobisConstantTerm = mahalanobisConstantTerm)
```
...
This means that (on average) the probability of purchase increases by about 1.0% per 1 unit increase in quality.
...
```{r}
apc2 <- GetPredCompsDF(model=logitFit2, df=df2, numRowsToUse = nRowsForApcSample, mahalanobisConstantTerm = mahalanobisConstantTerm)
apc2[c("Input",  "Apc.Signed")]
```

This means that in this variation the probability of purchase increases (on average) by about 1.2% (vs. 1.0% in *Variation 1*) per 1-point increase in quality. The magnitude of the APC for price is also larger.

### Variation 3

In this variation, prices are distributed uniformly as in *Variation 1*, but quality varies more in the middle range of prices. Since the APC weights large transitions more than small ones, this gives more weight to transitions where the slope is large, so the APC is larger than in *Variation 1*.
...
df3 <- local({
  price <- sample(20:120, nWines, replace=TRUE)
  quality <- price * .4 + 22 + ifelse(55 <= price & price <= 85, 
                                      rnorm(nWines, sd=qualityNoiseStdDev*1.5),
                                      rnorm(nWines, sd=qualityNoiseStdDev))
  purchaseProbability <- inv.logit(priceCoef*(price - 70) + qualityCoef*(quality - 50)  )
  purchased  <- rbinom(n = length(purchaseProbability), size=1, prob=purchaseProbability)
  data.frame(Quality = quality, 
             Price = price, 
...
```{r}
df3Sample <- df3[sample.int(nrow(df3), size=nRowsForPlottingSample), ]
...
```{r}
ggplot(subset(df3Sample, Price %in% seq(20, 120, by=10))) + 
  geom_point(aes(x = Quality, y = PurchaseProbability, color = factor(Price)), 
             size = 3, alpha = 1) + 
  ggtitle("Quality vs. Purchase Probability at Various Prices")
```
...
```{r}
apc3 <- GetPredCompsDF(model=logitFit1, df=df3, numRowsToUse = nRowsForApcSample, mahalanobisConstantTerm = mahalanobisConstantTerm)
