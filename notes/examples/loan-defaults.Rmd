...
library(knitr)
knitr::opts_chunk$set(tidy = FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.width = 14)
...
This example is based on the training data set [found here](https://www.kaggle.com/c/GiveMeSomeCredit).
...
Remove some outliers to get a more manageable data set:
...
Here's how the features are distributed after the above filtering:
...
This is the [impact](impact.html) chart. Its units are changes in probability, so we can compare it across all of the inputs:
...
oneRow <- t(oneOriginalRowNumber[1,intersect(names(oneOriginalRowNumber), names(credit2))])
