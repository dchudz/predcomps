...
These renormalized weights are the ones returned from ```GetPairs``` by default, and used in ```get_apc``` by default:

```{r}
get_apc(function(df) return(df$u * df$v), exampleDF2, u="u", v="v", renormalizeWeights=TRUE)
```
