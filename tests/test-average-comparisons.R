test_that("GetAPC returns expected answer on small example", {
  set.seed(89890)
  ...
  apc <- GetAPC(function(df) return(df$u * df$v), exampleDF2, u="u", v="v")
  expect_that(apc, equals(3.854473, tol=1e-6)) 
})


test_that("GetAPCWithAbsolute returns expected answer on small example", {
  exampleDF <- data.frame(
    v=c(-5,-5,5,5),  
    u=c(10,20,10,20) 
  )[rep(c(1,2,3,4),rep(50,4)),]
  apcList <- GetAPCWithAbsolute(function(df) return(df$u * df$v), exampleDF, u="u", v="v")
